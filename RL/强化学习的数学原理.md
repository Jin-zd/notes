...
贝尔曼最优方程（元素形式）：
$$
\begin{aligned} v(s) &= \max_{\pi} \sum_{a} \pi(a|s) \left( \sum_{r} p(r|s,a)r + \gamma \sum_{s'} p(s'|s,a)v(s') \right), \quad s \in S \\ &= \max_{\pi} \sum_{a} \pi(a|s)q(s,a), \quad s \in S \end{aligned}
$$
其中：
...
贝尔曼最优方程：元素形式
$$
v_*(s) = \max_{\pi} \sum_{a} \pi(a|s) \left( \sum_{r} p(r|s,a)r + \gamma \sum_{s'} p(s'|s,a)v(s') \right), \quad \forall s \in \mathcal{S}
$$
贝尔曼最优方程：矩阵-向量形式
$$
...
$$
[f(\mathbf{v})]_s = \max_{\pi} \sum_a \pi(a|s) q(s, a),\quad s \in S
$$
### 3.4.1 预备知识：压缩映射定理
#### 3.4.1.1 一些概念
...
从贝尔曼最优性方程中可以清晰地看出：
$$
v(s) = \max_{\pi} \sum_a \pi(a|s) \left( \sum_{r} p(r|s,a)r + \gamma \sum_{s'} p(s'|s,a)v(s') \right)
$$
有三个关键因素：
* 奖励设计：$r$
* 系统模型：$p(s'|s, a)$
...
&= (r_{\pi_{k+1}} + \gamma P_{\pi_{k+1}} v_{\pi_k}) - (r_{\pi'_{k}} + \gamma P_{\pi'_{k}} v_k) \\
&\quad (\text{假设 } \pi'_{k} = \arg \max_{\pi}(r_{\pi} + \gamma P_{\pi} v_k)) \\
&\ge (r_{\pi'_{k}} + \gamma P_{\pi'_{k}} v_{\pi_k}) - (r_{\pi'_{k}} + \gamma P_{\pi'_{k}} v_k) \\
&\quad (\text{因为 } \pi_{k+1} = \arg \max_{\pi}(r_{\pi} + \gamma P_{\pi} v_{\pi_k})) \\
&= \gamma P_{\pi'_{k}} (v_{\pi_k} - v_k).
\end{aligned}
$$
由于 $v_{\pi_k} - v_k \ge 0$ 且 $P_{\pi'_{k}}$ 是非负的，我们有 $P_{\pi_k}(v_{\pi'_{k}} - v_k) \ge 0$，因此 $v_{\pi_{k+1}} - v_{k+1} \ge 0$。
...
如果我们使用数据并更有效地更新估计，我们得到一个新算法，称为蒙特卡洛探索开始：
...
对于每个片段，执行：
	...
	对于片段的每个步骤，$t = T - 1, T - 2, \dots, 0$，执行：
		...
		策略改进： $\pi(a|s_t) = 1 \text{ if } a = \arg \max_a q(s_t, a) \text{ and } \pi(a|s_t) = 0 \text{ otherwise}$
...
对于每个片段，执行：
	...
	每轮初始化：$g \leftarrow 0$
	对于片段的每一步，从 $t = T - 1, T - 2, \ldots, 0$ 开始执行：
		    $g \leftarrow \gamma g + r_{t+1}$
	...
	策略改进：令 $a^* = \arg\max_a q(s_t, a)$ 并且$$\pi(a|s_t) = \begin{cases} 1 - \frac{|A(s_t)|-1}{|A(s_t)|}\epsilon, & a = a^* \\ \frac{1}{|A(s_t)|}\epsilon, & a \neq a^* \end{cases}$$
